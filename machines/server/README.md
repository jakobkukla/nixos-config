...
sudo ssh-keygen -t rsa -b 4096 -f /mnt/root/persist/etc/ssh/ssh_host_rsa_key -N "" -C "root@nixos-$MACHINE"
sudo ssh-keygen -t ed25519 -f /mnt/root/persist/etc/ssh/ssh_host_ed25519_key -N "" -C "root@nixos-$MACHINE"
```


Install once to create /etc/ssh.
Add new pub host key to secrets.nix and rekey on an existing machine. Then commit and push.
Copy /etc/ssh to persist
